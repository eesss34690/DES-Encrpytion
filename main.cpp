...
string funcF(string &r,string &k){
    ...
    for (int i=0 ;i< 48 ;i+= 6){
        row = stoi((e.substr(i, 1) + e.substr(i + 5, 1)), nullptr, 2);
        col = stoi(e.substr(i + 1, 4) , nullptr, 2);
        temp= S[i/6][row][col];
...
string Cipher(string msg,string key){
    ...
    string l0=key.substr(0, 28), r0=key.substr(28, 28);
    for (int i=0;i<16;i++){
        int t=2;
        if (i==0||i==1||i==8||i==15){
            t=1;
        ...
        key= l0+r0;
        string sub= pSUBKEY(key);
        string pl0= ml;
        ml= mr;
        mr=funcXOR32(pl0,funcF(ml,sub));
    }
    string mix= mr+ ml;
    return bin2hex(finalP(mix));
}
