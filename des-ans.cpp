...
//  DES
//
//  Created by PQA mabook pro Hsinchu on 2021/3/18.
//
...
#include <sstream>
using namespace std;
long long int chartoint(string input)
{
    long long int ans= 0;
    for (int i= 0; i< 8; i++)
    {
...
void pSUBKEY(int &l,int &r){
    ...
    for (int t=0;t<48;t++){
        ...
        if (map[t]>28){ // 從右邊拿
            if (t>23){   // 放右邊
                ...
                cout<<"r+: "<<getB(r,map[t]-28)<<endl;
            }
            else{
                setB(hl,getB(r,map[t]-28),t+1);
                cout<<"l+: "<<getB(l,map[t]-28)<<endl;
            }
        }
        else{
            if (t>23){
                setB(hr,getB(l,map[t]),t-23);
                cout<<"r+: "<<getB(r,map[t])<<endl;
            }
            else{
                setB(hl,getB(l,map[t]),t+1);
                cout<<"l+: "<<getB(l,map[t])<<endl;
            }
        }
    ...
    r=hr;
    l=hl;
    cout<<"Key shift not yet:= "<<bitset<24>(l)<<" "<<bitset<24>(r)<<endl;
    r=r>>8;
    r=r<<8;
    l=l>>8;
    l=l<<8;
...
long long int Cipher(long long int msg,long long int key){
    int keyr=(int)(key);
    int keyl=(int)(key>>32);
    cout<<"Key Left Int:= "<<bitset<32>(keyl)<<"  Key Right Int:= "<<bitset<32>(keyr)<<endl;
    int c0=0,d0=0;
    pKEY(keyl,keyr);
    c0=keyl;
    d0=keyr;
    cout<<"Key Left Int2:= "<<bitset<28>(keyl)<<"  Key Right Int2:= "<<bitset<28>(keyr)<<endl;
    int kl=0,kr=0;
    int r0=(int)(msg);
    int l0=(int)(msg>>32);
    pIP(l0,r0);
    for (int i=1;i<17;i++){
        ...
        kl=c0;
        kr=d0;
        cout<<"Key shift pre:= "<<bitset<28>(kl)<<" "<<bitset<28>(kr)<<endl;
        pSUBKEY(kl,kr);
        cout<<"Key shift l:= "<<bitset<24>(kl)<<bitset<24>(kr)<<endl;
        int pl0=l0;
        l0=r0;
...
int main (){
    ...
    ofstream ofs("output-ans.txt");
    string k, m, temp;
    while (getline(ifs, temp))
    {
        k= temp.substr (0,8);
        m= temp.substr (9,8);
        long long int msg=chartoint(m);
        long long int key=chartoint(k);
        ofs<<hex<<Cipher(msg,key)<<endl;
    }
    return 0;
}
